...
<div class="container-fluid">
    <div class="row-fluid">		
	     ...
	     <div class="span6" id="clientajaxcontainer">
		  ...
		  <h2>MAC Authentication Status: Port <?php echo $portname;?></h2>
		      <p>
			    <?php echo anchor("switches/details/$objectid","Switch Details Page");?>
			  
			    <br><br> 

		      <?php echo ($macAuthStatus); ?>
			<BR><BR>
			  <button class="btn enable">Enable MAC Auth</button>&nbsp;
			  <button class="btn disable">Disable MAC Auth</button>		      	 
	     </div>      
...
$(document).ready(function(){
      ...
      $('.enable').live('click', function()  {
				  ...
				  var htmlstring;
				  $.ajax({
						    url:"<?php echo site_url('switches/macauthOn/'.$dnsName.'/'.$hardwaremodel.'/'.$portname.'/'.$FQDN);?>",
						    type:'POST',
						    dataType:'text',
						    success: function(returnDataFromController) {						    
							htmlstring = "<h2>MAC Authentication Status: Port " + $('#port').val() + "</h2><a href=" + BASEPATH + "index.php/switches/details/" + $('#objectid').val() + ">Switch Details Page</a><p><p>" + returnDataFromController + "<BR><BR><button class='btn enable'>Enable MAC Auth</button>&nbsp;<button class='btn disable'>Disable MAC Auth</button>";	
							//alert(htmlstring);
      ...
      $('.disable').live('click', function()  {
				...
				$.ajax({
						  url:"<?php echo site_url('switches/macauthOff/'.$dnsName.'/'.$hardwaremodel.'/'.$portname.'/'.$FQDN);?>",
						  type:'POST',
						  dataType:'text',
						  success: function(returnDataFromController) {
							htmlstring = "<h2>MAC Authentication Status: Port " + $('#port').val() + "</h2><a href=" + BASEPATH + "index.php/switches/details/" + $('#objectid').val() + ">Switch Details Page</a><p><p>"  + returnDataFromController + "<BR><BR><button class='btn enable'>Enable MAC Auth</button>&nbsp;<button class='btn disable'>Disable MAC Auth</button>";	
